...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%1
	...
	To znaczy, że musimy sprawdzać kolejne przedziały $ \left( \sum\limits_{i=1}^{j-1} p_i, \sum\limits_{i=1}^{j} p_i \right] $, czy wpada do nich $U$. W przypadku, gdy $\lambda$, czyli średnia wartość rozkładu zmiennej $X$, jest duża, przechodzenie przez wszystkie przedziały może zajmować sporo czasu. Żeby skrócić czas wykonania, możemy zacząć sprawdzanie od przedziału $ \left( \sum\limits_{i=1}^{\lceil \lambda \rceil -1} p_i, \sum\limits_{i=1}^{\lceil \lambda \rceil} p_i \right] $. Następnie, jeśli $U$ jest mniejsze, to sprawdzamy przedziały na lewo, jeśli większe to na prawo. Dodatkowo zauważmy, że
	$$ p_{i + 1} = p_i \cdot \frac{\lambda}{i + 1}, $$
	Stąd kolejne $p_i$ otrzymamy domnażając $\frac{\lambda}{i + 1}$. Algorytm:
	\begin{enumerate}[leftmargin=10mm]
		\item Oblicz wartości $p_i$ dla $i = 0, 1, \dots, \lceil\lambda\rceil$.
		\item Zdefiniuj $j = \lceil \lambda \rceil$, \ $p = p_{\lceil\lambda\rceil}$, \ $F = p$.
		\item Generuj $U \sim \mathcal{U}(0, 1)$.
	\end{enumerate}
