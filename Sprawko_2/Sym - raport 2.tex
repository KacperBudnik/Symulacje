...
\begin{document}
	...
	gdzie $B_n(u) = \mathbb{P}(Y_1 + \dots + Y_n > u)$ i $Y_i$ to zmienne losowe iid o gęstości $f(x) = \frac{1 - F_X(x)}{\mu}$, a $F_X(x)$ jest dystrybuantą rozkładu strat $X_i$. Wykorzystamy pewien fakt wynikający z tego wzoru, a konkretnie to, że
	...
	\begin{enumerate}[leftmargin=10mm]
		...
		\item Generuj $Y_1, \dots, Y_K$ z rozkładu o gęstości $f(x) = \frac{1 - F_X(x)}{\mu}$.
		\item Jeśli $Y_1 + \dots + Y_K > u$, to $Z = Z + 1$.
		\item Kroki 2, 3, 4 powtórz $N$ razy.
		\item Zwróć $\Psi(u) = \frac{Z}{N}$.
	\end{enumerate}
	Po wykonaniu tego algorytmu dla $N = 1000$ powtórzeń, otrzymaliśmy
	$$ \Psi(50) = .$$
...
%	$B^x_t\in[a,b]\iff B^y_t\in[a-x+y, b-x+y]$\\widzimy, że czas wyjścia nie zależy bezpośrednio od wyboru punktów $a$ oraz $b$, a od punktu początkowego oraz długości przedziału.
	...
	\end{equation}
	gdzie $\widetilde\tau^y$ jest czasem wyjścia z przedziału $[0, 1]$ jeśli zaczniemy z punktu $y=\frac{x-a}{b-a}$, czyli z odcinek dzielącego przedział $[0, 1]$ w stosunku takim samym jak $x$ dzieli odcinek $[a, b]$.\\
